...
class IS_Search_Editor
{
    ...
    public function includes_panel( $post )
    {
        ...
        foreach ( $post_types2 as $post_type ) {
            ...
            if ( !$posts_found ) {
                $html .= '<br /><span class="notice-is-info">' . sprintf( __( 'No %s created.', 'add-search-to-menu' ), $post_types[$post_type]->labels->name ) . '</span>';
            } else {
                $html .= '<br /><label for="' . $id . '-post__in" class="ctrl-multi-select">' . esc_html__( "Hold down the control (ctrl) or command button to select multiple options.", 'add-search-to-menu' ) . '</label><br />';
            }
